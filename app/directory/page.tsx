    ...
    <div className="min-h-screen bg-[#F7F6F3] text-[#141412]">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {CITIES.map((c) => (
              <Link key={c.slug} href={`/clothes-shops/${c.slug}`} className="text-sm text-[#57554E] hover:text-[#141412] bg-white hover:bg-[#EAE8E3] border border-[#E3E1DB] px-3 py-1.5 rounded-full transition-colors">
                {c.name}
              </Link>
            ))}
          </div>
