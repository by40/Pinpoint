          ...
          <div className="p-4 border-b border-line bg-surface space-y-3">
            <form onSubmit={(e) => { e.preventDefault(); addItem(); }} className="flex gap-2">
              <input
                ...
                className="flex-1 bg-surface border border-line text-ink placeholder-faint rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-accent/40 focus:ring-1 focus:ring-accent/25 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              />
              <motion.button
                type="submit"
                disabled={!input.trim() || atItemLimit}
                whileTap={{ scale: 0.97 }}
                className="bg-accent hover:bg-accent-hover disabled:opacity-30 disabled:cursor-not-allowed text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-colors"
              >
                Add
              </motion.button>
            </form>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label htmlFor="wl-radius" className="text-xs text-muted">Search radius</label>
              ...
              <div className="flex flex-col items-center justify-center min-h-[50vh] md:h-full text-center py-10 px-6">
                ...
                >
                  <div className="w-12 h-12 rounded-xl bg-surface border border-line flex items-center justify-center mx-auto">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8a8794" strokeWidth="1.5">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-ink mb-1">Your list is empty</h3>
                    <p className="text-xs text-faint leading-relaxed max-w-[200px]">
                      Add clothes above, then find shops for the whole outfit at once.
                    </p>
                  </div>
                  <div className="pt-1">
                    <p className="text-[9px] text-faint uppercase tracking-widest mb-2">Try these</p>
              ...
              <div className="p-4 space-y-1.5">
                ...
                <AnimatePresence>
                  {searched && !totalLoading && (
                    <motion.div
                      ...
                      exit={{ opacity: 0 }}
                      className="mb-3 p-3.5 rounded-xl border border-line bg-surface flex items-center justify-between"
                      role="status"
                      aria-live="polite"
                    >
                      <div>
                        ...
                        <p className="text-xs text-faint mt-0.5">within {radiusMiles} {radiusMiles === 1 ? "mile" : "miles"} · tap an item to see shops</p>
                      </div>
                      <div className={`text-xs font-bold tabular-nums ${coveredCount === items.length ? "text-ink" : "text-faint"}`}>
                        {coveredCount}/{items.length}
                      </div>
                    </motion.div>
                  )}
                  ...
                  {items.map((item, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span className="text-faint text-xs tabular-nums w-4 shrink-0 text-right">{i + 1}.</span>
                          <span className="text-sm font-medium flex-1 min-w-0 truncate">{item.text}</span>

                          {result?.loading && (
                            <svg className="animate-spin h-3.5 w-3.5 text-faint shrink-0" viewBox="0 0 24 24" fill="none">
                              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
                            </svg>
                          )}
                          {result && !result.loading && (
                            <span className={`text-xs shrink-0 ${result.shops.length > 0 ? "text-ink font-medium" : "text-faint"}`}>
                          ...
                          <svg
                            ...
                            className={`shrink-0 text-faint transition-transform duration-200 ${isActive ? "rotate-90" : ""}`}
                          >
                          ...
                          <button
                            ...
                            className="shrink-0 text-faint hover:text-accent transition-colors text-base leading-none ml-0.5"
                          >
                            ×
                          </button>
                        </div>
                            ...
                            >
                              <div className="pt-1.5 pl-3 space-y-1.5">
                                {result.shops.length === 0 && (
                                  <p className="text-xs text-faint py-2 px-3">No shops found within {radiusMiles} {radiusMiles === 1 ? "mile" : "miles"}. Try a broader radius or term.</p>
                                )}
                                ...
                                {result.shops.length > 6 && (
                                  <p className="text-xs text-faint text-center py-1">
                ...
                {unusedSuggestions.length > 0 && (
                  <div className="pt-3 pb-1">
                    <p className="text-[9px] text-faint uppercase tracking-widest mb-2">Quick add</p>
                    <div className="flex flex-wrap gap-1.5">
                      {unusedSuggestions.slice(0, 6).map((s) => (
                        <button
                          key={s}
                          onClick={() => addItem(s)}
                          className="text-[11px] bg-surface hover:bg-panel border border-line text-muted hover:text-accent px-2.5 py-1 rounded-full transition-colors"
                        >
                          + {s}
                        </button>
                      ))}
                ...
                <div className="pt-3">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full bg-accent hover:bg-accent-hover disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl text-sm transition-colors flex items-center justify-center gap-2"
                  >
                    {locating || searching ? (
                      <>
                  ...
                  {manualMode && (
                    ...
                    >
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.97 }}
                        className="bg-accent hover:bg-accent-hover disabled:opacity-30 disabled:cursor-not-allowed text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-colors whitespace-nowrap"
                      >
                  ...
                  {geoError && (
                    <p className="mt-2 text-muted text-xs" role="alert">{geoError}</p>
                  )}
                </div>
              </div>
            )}
        ...
        <div className="relative h-[60vh] md:h-auto md:flex-1 md:min-h-0 bg-panel border-t md:border-t-0 border-line">
          {!searched && (
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none gap-2">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#8a8794" strokeWidth="1.5">
                <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2" />
                <rect x="9" y="3" width="6" height="4" rx="1" />
                <line x1="9" y1="12" x2="15" y2="12" />
                <line x1="9" y1="16" x2="13" y2="16" />
              </svg>
              <p className="text-faint text-sm">Add items and tap &ldquo;Find shops&rdquo;</p>
            </div>
          )}
                ...
                return (
                  ...
                  >
                    ...
                    {res && !res.loading && res.shops.length > 0 && (
                      <span className={`ml-1.5 ${isActive ? "text-white/60" : "text-faint"}`}>
                        {res.shops.length}
                      </span>
                    )}
                    {res?.loading && (
